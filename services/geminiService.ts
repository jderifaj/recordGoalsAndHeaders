...
export const getCoachAnalysis = async (
  ...
  const drillStats = session.reps
    .map((r) => {
      if (r.drillType === DrillType.SHOOTING) {
        const footStr = r.foot
          ? ` using their ${r.foot === "R" ? "Right" : "Left"} foot`
          : "";
        return `- Shooting (${r.exerciseName})${footStr}: ${r.shotsMade}/${r.shotsTaken} goals to ${r.targetArea}`;
      } else {
  ...
  const ageContext =
    age > 0 ? `The athlete is ${age} years old.` : "Age unknown.";
  ...
  const prompt = `
    ...
    Location: ${session.location || "Unknown Field"}
    Exercises Recorded:
    ${drillStats}
    ...
    3. Evaluate if their scores are "good" relative to their age and session type. Mention the foot usage (Right vs Left) if shooting drills were recorded, emphasizing the importance of two-footed play.
